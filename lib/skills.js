...
const VALID_SKILLS = new Set(['all', 'projectmap', 'reflections', 'simplicity', 'changelimit']);
...
export class SkillManager {
  ...
  async execute(platform, action, skill, isGlobal = false) {
    if (!platform) return console.error('[Code-Graph] Platform required. Usage: code-graph install-skills <platform> [projectmap|reflections] [-g]');
  ...
  async uninstall(p, s, isGlobal = false) {
    ...
    try {
      ...
      if (isGlobal) {
        ...
        if (s === 'all' || s === 'simplicity') {
          await this.removeSkillArtifacts(p, 'simplicity', this.simplicitySpec().section);
        }
        if (s === 'all' || s === 'changelimit') {
          await this.removeSkillArtifacts(p, 'changelimit', this.changeLimitSpec().section);
        }
