...
const VALID_SKILLS = new Set(['all', 'projectmap', 'reflections', 'simplicity', 'changelimit', 'freshdeps']);
...
export class SkillManager {
  ...
  async execute(platform, action, skill, isGlobal = false) {
    if (!platform) return console.error('[Code-Graph] Platform required. Usage: code-graph install-skills <platform> [projectmap|reflections|simplicity|changelimit|freshdeps] [-g]');
    ...
    if (!VALID_SKILLS.has(s)) {
      return console.error(`[Code-Graph] Unknown skill: ${skill}. Valid: projectmap, reflections, simplicity, changelimit, freshdeps, all`);
    }
