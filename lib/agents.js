...
export class AgentManager {
  ...
  async install(p) {
    ...
    try {
      switch (p) {
        ...
        case 'claude':
          await this.installMCPServer('.mcp.json');
          await this.installClaudeSubagent();
          break;
    ...
    try {
      switch (p) {
        case 'gemini':
          ...
          break;
        case 'claude':
          await this.removeMCPServer('.mcp.json');
        ...
        case 'antigravity':
          ...
          await this.removeMCPServerAbsolute(path.join(this.home, '.gemini', 'antigravity', 'mcp_config.json'));
          // Legacy path cleanup
  ...
  async installMCPServer(relPath) {
    await this.installMCPServerAbsolute(path.join(this.cwd, relPath));
  }

  async installMCPServerAbsolute(target) {
    const entry = {
      command: "node",
    ...
    try {
      ...
      if (e.code !== 'ENOENT') {
        console.warn(`[Code-Graph] Warning: couldn't parse ${target}, overwriting: ${e.message}`);
      }
    }
  ...
  async removeMCPServer(relPath) {
    await this.removeMCPServerAbsolute(path.join(this.cwd, relPath));
  }

  async removeMCPServerAbsolute(target) {
    try {
  ...
  async installClaudeSubagent() {
    ...
    const content = `---\nname: code-graph\ndescription: Codebase mapping and memory specialist. Delegate here for architectural overviews, refreshing the project map with \`code-graph generate\`, or persisting non-obvious lessons via \`code-graph reflect\`.\ntools: Read, Grep, Glob, Bash\nmodel: inherit\n---\nYou are the Code-Graph Specialist.\n\nResponsibilities:\n1. Before searching raw files, read \`llm-code-graph.md\` for god nodes and structural context.\n2. If the map looks stale or missing, run \`code-graph generate\` to refresh it.\n3. Read \`llm-agent-project-learnings.md\` for past lessons that apply to the current task.\n4. After resolving a non-obvious issue, record a lesson via \`code-graph reflect <category> <one-sentence lesson>\`.\n5. Return a concise summary to the main conversation — not raw exploration output.\n`;
  ...
  async installAntigravityAgent() {
    ...
    const skillContent = `---\nname: code-graph\ndescription: Specialized analyst for codebase mapping and memory persistence. Use \`code-graph generate\` to refresh the project map and \`code-graph reflect\` to save lessons.\n---\n# Code-Graph Agent\nRole: Specialized analyst for codebase mapping and memory persistence.\nCapabilities: Run \`code-graph generate\` to refresh the project map and \`code-graph reflect\` to persist non-obvious lessons.\nUsage: Delegate architectural or environmental analysis here.\n`;
    await fsp.writeFile(path.join(skillDir, 'SKILL.md'), skillContent);

    // MCP server registration — the canonical way to expose code-graph as a tool to the Antigravity agent.
    await this.installMCPServerAbsolute(path.join(this.home, '.gemini', 'antigravity', 'mcp_config.json'));
  }
