...
- lib/skills.js (5↑ 2↓) | d: @file lib/skills.js @description Manages platform-specific skills and agent inte
  - s: [SkillManager, appendToFile [(filename, content)], changeLimitBody [()], changeLimitSpec [()], contextBudgetBody [()], contextBudgetSpec [()], execute [(platform, action, skill)], freshDepsBody [()], freshDepsSpec [()], install [(p, s)], installGlobalSkill [(platform, skillName, description, body)], installSkill [Platform-dispatcher driven by a per-skill spec. Each spec declares which artifacts apply; platforms pick what's relevant. /], projectMapBody [()], projectMapSection [()], projectMapSpec [()], reflectionsProtocolBody [()], reflectionsSection [()], reflectionsSpec [()], removeFile [(filename)], removeFileIfEmpty [(filename)], removeFromFile [(filename, content)], removeJsonArrayValue [(filename, key, value)], removeJsonHookEntry [(filename, shouldRemove)], removeLegacySkills [(p)], removeOldReflectionsPrompts [()], removeSkillArtifacts [Remove all artifacts that installSkill creates across platforms for a given skill name + section. Safe to call regardless of which platform originally installed it.], roocodeReflectionsSection [()], simplicityBody [()], simplicitySpec [()], uninstall [(p, s)], writeFile [(filename, content)], writeJson [(filename, data)]]
